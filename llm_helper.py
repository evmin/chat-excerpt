...
class LLMHelper:
    ...
    def get_chat_completion_with_functions(
        ...
        message_list = [{"role": chat["role"], "content": chat["content"]} for chat in messages]
        message_list_dimension = {
            "messages": json.dumps(message_list)
        }
        logger.info("LLM PROMPT func", extra=message_list_dimension)
    ...
    def get_chat_completion(
        ...
        custom_dimensions = {f"message.{i}": json.dumps(message) for i, message in enumerate(messages)}
        logger.info("LLM PROMPT", extra=custom_dimensions)
